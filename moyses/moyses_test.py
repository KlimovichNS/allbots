...
def send_stat_one_head(BK,user_id,df,a):
    ...
    if name=='error':
        ...
        bot.send_message(user_id, text='Проверьте правильность введенного кода главы. Обратите внимание, код главы - трехзначный', reply_markup=markup, parse_mode= 'Markdown')
    else:
        foiv = df.query('БК == @BK')
...
def Get_Table():
    ...
    df  = df[['Код по СВР', 'GUID организации','ФОИВ','Код БК' , 'КРАТКОЕ НАИМЕНОВАНИЕ ОРГАНИЗАЦИИ', 'База получена','дата получения базы','проблемы (база при получении)', 'Загружено из ЕИСУКС','дата загрузки из ЕИСУКС', 'проблемы при загрузке из ЕИСУКС ШР','кол-во ошибок по отчету ЕИСУКС сотрудники','Количество назначенных ЛД','Дата подписания акта','дата загрузки дампа в ЭБ', 'Протокол (полож, отр)', 'заявка на перемиграцию', 'Загружено из ВИС','проблемы загрузки из ВИС','Дата протокола ВИС','Признак','Акт приема-передачи']]
    df.columns = ['СВР', 'GUID', 'фоив', 'БК', 'наименование', 'получена', 'дата получения','проблема(получение)','загружено','дата ЕИСУКС', 'проблемы','ошибки(сотрудники)','кол-во назначенных','дата акта п/п','дата дампа', 'протокол', 'перемиграция', 'загружено_ВИС','проблемы_ВИС','дата протокола ВИС','признак','акт п/п']
...
def get_values_SVR(df):
    ...
    array_values.append(str(df.iat[0,14])+'\n')
